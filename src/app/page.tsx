    ...
    <div className="flex flex-col gap-4 bg-gray-100 min-h-screen">
      ...
      <main className="px-3 max-w-7xl max-auto flex flex-col gap-9 w-full pb-10 pt-4">
            ...
            <Container className=" gap-10 px-6 items-center">
              ...
              <div className="flex flex-col px-4">
                <span className="text-5xl">
                  {convertKelvinToCelsisus(firstData?.main.temp ?? 0)}°
                </span>
                <p className="text-xs space-x-1 whitespace-nowrap">
                  <span> Feels like</span>
                  <span>
                    {convertKelvinToCelsisus(firstData?.main.feels_like ?? 0)}°
                  </span>
                </p>
                <p className="text-xs space-x-2">
                  <span>
                    {convertKelvinToCelsisus(firstData?.main.temp_min ?? 0)}°↓{" "}
                  </span>
                  <span>
                    {" "}
                    {convertKelvinToCelsisus(firstData?.main.temp_max ?? 0)}°↑
                  </span>
                </p>
              </div>
              ...
              <div className="flex gap-10 sm:gap-16 overflow-x-auto w-full justify-between pr-3">
                {data?.list.map((d, i) => (
                  <div
                    key={i}
                    className="flex flex-col justify-between gap-2 items-center text-sx font-bold"
                  >
                    ...
                    />
                    <p>{convertKelvinToCelsisus(d?.main.temp ?? 0)}°</p>
                  </div>
                ))}
              </div>
            </Container>
        ...
        <section className="flex w-full flex-col gap-4">
          <p className="text-2xl">Forcase (7 days)</p>
        </section>
      </main>
    </div>
  );
